...
if(isset($_POST['keyword'])) {
    ...
    foreach ($list as $res) {
        $ListeCava = str_replace($_POST['keyword'], '<b>'.$_POST['keyword'].'</b>', $res['nomcava']);
        // Utilise l'index `id` pour l'élément HTML, qui est passé depuis la fonction autocomplet() dans le JavaScript
        echo '<li onclick="set_item(\''.str_replace("'", "\'", $res['nomcava']).'\', \''.$_POST['index'].'\', '.$res['idcava'].', \'cava\')">'.$ListeCava.'</li>';
    }
}
?>

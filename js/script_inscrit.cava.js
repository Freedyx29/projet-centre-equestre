// autocompletion
function autocompletInscritCava(id) {
    var min_length = 1;
    var keyword = $('#nomcava' + id).val();

    if (keyword.length >= min_length) {
        $.ajax({
            url: '../ajax/ajax.inscrit.idcava.php', 
            type: 'POST',
            data: { 
                keyword: keyword,
            ...
            success: function(data) {
                $('#nom_list_inscrit_cava_id' + id).show();
                $('#nom_list_inscrit_cava_id' + id).html(data);
            },
        });
    } else {
        $('#nom_list_inscrit_cava_id' + id).hide();
    }
}

function autocompletInscritCavaajout() {
    var min_length = 1;
    var keyword = $('#nomcava').val();

    if (keyword && keyword.length >= min_length) {
        $.ajax({
            url: '../ajax/ajax.inscrit.idcavaajout.php', 
            type: 'POST',
            data: { 
                keyword: keyword,
            },
            success: function(data) {
                $('#nom_list_inscrit_cava_id').show();
                $('#nom_list_inscrit_cava_id').html(data);
            },
        });
    } else {
        $('#nom_list_inscrit_cava_id').hide();
    }
}

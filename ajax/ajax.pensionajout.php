...
if(isset($_POST['keyword'])) {
    ...
    foreach ($list as $res) {
        $Listepensionajout = str_replace($_POST['keyword'], '<b>'.$_POST['keyword'].'</b>', $res['nomche']);
        // Utilise l'index `id` pour l'élément HTML, qui est passé depuis la fonction autocomplet() dans le JavaScript
        echo '<li onclick="set_item_pension_ajout(\''.str_replace("'", "\'", $res['nomche']).'\',  '.$res['numsire'].')">'.$Listepensionajout.'</li>';
    }
}
?>
